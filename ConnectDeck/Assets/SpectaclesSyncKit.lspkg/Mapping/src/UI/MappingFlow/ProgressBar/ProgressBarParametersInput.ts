...
export class ProgressBarParametersInput extends BaseScriptComponent {
  ...
  @input("float", "0.5")
  @hint(
    "The duration in seconds for the progress bar to gradually fade in (via alpha)"
  )
  readonly fadeInTime: number

  @input("float", "0.3")
  @hint(
    "The duration in seconds for the progress bar to gradually fade out (via alpha)"
  )
